...
class Conductor:
    ...
    async def submit(self, wrapped_cmd: str) -> dict:
        ...
        if self.submission_stage == "detection":
            ...
            self.logger.info(f"[EVAL] Detection {"Succeed" if self.results["Detection"]["success"] else "Failed"}\n TTD: {self.results['TTD']}")
        ...
        if self.submission_stage == "localization":
            ...
            self.logger.info(f"[EVAL] Localization {"Succeed" if self.results["Localization"]["success"] else "Failed"}\n TTL: {self.results['TTL']}")
        ...
        if self.submission_stage == "mitigation":
            ...
            self.logger.info(f"[EVAL] Mitigation {"Succeed" if self.results["Mitigation"]["success"] else "Failed"}\n TTM: {self.results['TTM']}")
