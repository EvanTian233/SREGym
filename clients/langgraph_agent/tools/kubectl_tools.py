...
class ExecKubectlCmdSafely(BaseTool):
    ...
    async def _arun(
        ...
        return Command(
            update={
                "messages": [
                    ToolMessage(content=text_result,
    ...
    async def _arun(
        ...
        for c in kubectl_read_only_cmds:
            if command.startswith(c):
                ...
                break
        if not is_read_only:
            logger.info(f"Agent is trying to exec a non read-only command {command} "
                        f"with tool exec_read_only_kubectl_cmd")
            text_result = f"Your command {command} is not a read-only kubectl command. " \
                          f"Available Read-only Commands: {kubectl_read_only_cmds}."
