        ...
        if 'gmail.com' in self.smtp_server.lower():
            if not self.is_gmail_app_password(self.password):
                raise ValueError(
                    ...
                    "Please generate one from your Google Account settings: "
                    "Google Account > Security > App Passwords"
                )
            
    def is_email_set(self) -> bool:
        return self.username is not None and self.password is not None and self.smtp_server is not None and self.smtp_port is not None
